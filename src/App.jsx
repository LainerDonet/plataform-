...
export const PaginatedTableAction = () => {
  ...
  const columns = [translation.table.columns.servers, translation.table.columns.status, translation.table.columns.location, translation.table.columns.modified, translation.table.columns.url];
  const numRows = 25;
  ...
  const createRows = () => {
    ...
    for (let i = 0; i < numRows; i++) {
      ...
      const rowObj = {
        ...
        status: [translation.table.rows.status.stopped, translation.table.rows.status.running, translation.table.rows.status.down, translation.table.rows.status.needsMaintenance][getRandomInteger(0, 3)],
        location: [translation.table.rows.locations.raleigh, translation.table.rows.locations.boston, translation.table.rows.locations.atlanta, translation.table.rows.locations.sanFrancisco][getRandomInteger(0, 3)],
        lastModified: [translation.table.rows.lastModified.oneHr, translation.table.rows.lastModified.threeHrs, translation.table.rows.lastModified.fiveHrs, translation.table.rows.lastModified.sevenMins, translation.table.rows.lastModified.fortyTwoMins, translation.table.rows.lastModified.twoDays, translation.table.rows.lastModified.oneMonth][getRandomInteger(0, 6)],
  ...
  const renderPagination = variant => {
    ...
    return <Pagination itemCount={managedRows.length} page={page} perPage={perPage} onSetPage={handleSetPage} onPerPageSelect={handlePerPageSelect} variant={variant} titles={{
  ...
  const renderLabel = labelText => {
    switch (labelText) {
      case 'Running':
      case 'רץ':
        return <Label color="green" icon={<Icon shouldMirrorRTL>
                <WalkingIcon />
              </Icon>}>
      ...
      case 'Stopped':
      case 'עצר':
        return <Label icon={<Icon shouldMirrorRTL>
                <HandPaperIcon />
              </Icon>} color="red">
      ...
      case 'זקוק לתחזוקה':
        return <Label icon={<ToolsIcon />} color="blue">
            {translation.table.rows.status.needsMaintenance}
          </Label>;
      case 'Down':
      case 'מטה':
        return <Label icon={<ClockIcon />} color="orange">
        ...
        <ToolbarContent>
          <ToolbarItem>
            <Button variant="primary" icon={<Icon shouldMirrorRTL>
                  <AlignRightIcon />
                </Icon>} iconPosition="end" onClick={switchTranslation}>
  ...
  const pageNav = <Nav aria-label={translation.nav.ariaLabel || undefined}>
      <NavList>
  ...
  const masthead = <Masthead>
    ...
    lg: 'visible'
                ...
                <ToolbarItem>
                  <Button aria-label={translation.kebabDropdown.settings} variant={ButtonVariant.plain} icon={<CogIcon />} />
                </ToolbarItem>
                <ToolbarItem>
                  <Button aria-label={translation.kebabDropdown.help} variant={ButtonVariant.plain} icon={<QuestionCircleIcon />} />
                </ToolbarItem>
              </ToolbarGroup>
    ...
    lg: 'hidden'
                ...
                <Dropdown isOpen={isKebabDropdownOpen} onSelect={onKebabDropdownSelect} onOpenChange={setIsKebabDropdownOpen} popperProps={{
    position: 'right'
  }} toggle={toggleRef => <MenuToggle ref={toggleRef} isExpanded={isKebabDropdownOpen} onClick={onKebabDropdownToggle} variant="plain" aria-label={translation.kebabDropdown.settingsAndHelp}>
    ...
    md: 'hidden'
                ...
                <Dropdown isOpen={isFullKebabDropdownOpen} onSelect={onFullKebabSelect} onOpenChange={setIsFullKebabDropdownOpen} popperProps={{
    position: 'right'
  }} toggle={toggleRef => <MenuToggle ref={toggleRef} isExpanded={isFullKebabDropdownOpen} onClick={onFullKebabToggle} variant="plain" aria-label={translation.kebabAndUserDropdown.toolbarMenuAriaLabel}>
                      <EllipsisVIcon aria-hidden="true" />
                    </MenuToggle>}>
        ...
        <PageSection>
          <Card>
            ...
            <Table variant="compact" aria-label={translation.table.ariaLabel}>
              <Thead>
                <Tr>
                  {columns.map((column, columnIndex) => <Th key={columnIndex}>{column}</Th>)}
                </Tr>
              </Thead>
              <Tbody>
                {paginatedRows.map((row, rowIndex) => <Tr key={rowIndex}>
                    <>
                      {Object.entries(row).map(([key, value]) => {
    if (key === 'status') {
      return <Td key={key} width={15} dataLabel="Status">
                              {renderLabel(value)}
                            </Td>;
    } else if (key === 'url') {
      return <Td key={key} dataLabel="URL" width={15}>
                              <a href="#">
                                <Truncate content={row.url} position={isDirRTL ? 'end' : 'start'} />
                              </a>
                            </Td>;
    } else {
      return <Td key={key} dataLabel={key === 'lastModified' ? 'Last modified' : capitalize(key)}>
                              {value}
                            </Td>;
    }
  })}
...
export default PaginatedTableAction
